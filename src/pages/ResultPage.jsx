...
function mapScoresToFeatures(scores) {
  ...
  return [
    n(scores.values.성취),                                                              // 성취/노력
    n(scores.aptitude.자기관리능력),                                                      // 인내
    n(scores.values.도전성),                                                             // 책임성과 진취성
    n((scores.interest.E + scores.aptitude.대인관계능력) / 2),                           // 리더십 (E형 + 대인관계)
    n((scores.interest.A + scores.interest.I + scores.aptitude.창의력) / 3),            // 혁신 (A형 반영)
    n((scores.aptitude.대인관계능력 + scores.values.사회적기여 + scores.interest.S) / 3), // 배려 (S형 반영)
  ]
}
...
function getJobRiasec(name) {
  // R형
  if (/기계|전자|전기|설비|제조|생산|용접|배관|목공|목수/.test(name)) return ['R', 'I']
  if (/건축|토목|건설|인테리어/.test(name)) return ['R', 'A']
  if (/요리|조리|제빵|셰프|식품|조리사/.test(name)) return ['R', 'A']
  if (/농업|임업|어업|원예|조경|동물/.test(name)) return ['R', 'S']
  if (/운전|운송|물류|정비/.test(name)) return ['R', 'C']
  if (/개발자|프로그래머|소프트웨어|시스템/.test(name)) return ['I', 'R']
  // I형
  if (/연구원|연구자|과학자|물리|화학|생물|지질/.test(name)) return ['I', 'R']
  if (/데이터|통계|분석가/.test(name)) return ['I', 'C']
  if (/의사|의학|약사|치과|한의/.test(name)) return ['I', 'S']
  if (/경제학|사회학|심리학/.test(name)) return ['I', 'S']
  // A형
  if (/디자이너|디자인|UX|UI|그래픽|패션/.test(name)) return ['A', 'I']
  if (/화가|조각|공예|미술작가|화백/.test(name)) return ['A', 'R']
  if (/음악가|작곡가|연주자|가수|성악|뮤지션|피아니스트|바이올린/.test(name)) return ['A', 'S']
  if (/영화감독|연출|배우|연기자|성우|탤런트/.test(name)) return ['A', 'S']
  if (/소설가|시인|작가|웹툰|만화가|스토리/.test(name)) return ['A', 'I']
  if (/사진작가|촬영|포토/.test(name)) return ['A', 'R']
  if (/무용|댄서|안무/.test(name)) return ['A', 'S']
  if (/크리에이터|유튜버|콘텐츠/.test(name)) return ['A', 'E']
  if (/애니메이션|일러스트/.test(name)) return ['A', 'I']
  // S형
  if (/교사|교수|강사|유아교사|보육|코치/.test(name)) return ['S', 'A']
  if (/상담사|심리사|치료사|상담/.test(name)) return ['S', 'I']
  if (/사회복지|복지사/.test(name)) return ['S', 'E']
  if (/간호사|간호/.test(name)) return ['S', 'I']
  if (/물리치료|재활/.test(name)) return ['S', 'R']
  if (/경찰|소방|군인|보안|경호/.test(name)) return ['S', 'E']
  // E형
  if (/마케터|마케팅|홍보|광고|PR/.test(name)) return ['E', 'A']
  if (/기자|언론|방송작가|PD|아나운서|MC/.test(name)) return ['E', 'A']
  if (/컨설턴트|컨설팅/.test(name)) return ['E', 'C']
  if (/부동산|보험|영업|세일즈/.test(name)) return ['E', 'C']
  if (/경영|CEO|창업/.test(name)) return ['E', 'I']
  // C형
  if (/회계사|세무사|재무/.test(name)) return ['C', 'E']
  if (/금융|은행원|투자|증권/.test(name)) return ['C', 'E']
  if (/공무원|행정/.test(name)) return ['C', 'S']
  if (/변호사|판사|검사|법무/.test(name)) return ['C', 'E']
  if (/사서|기록|문서/.test(name)) return ['C', 'I']
  return ['R', 'I']
}

function riasecBonus(jobRiasec, userTop2) {
  return jobRiasec.filter(r => userTop2.includes(r)).length
}

function getCareerMatches(scores) {
  const user = mapScoresToFeatures(scores)
  const userTop2 = Object.entries(scores.interest).sort(([,a],[,b]) => b-a).slice(0,2).map(([k]) => k)
  return JOB_PROFILES
    .map(job => {
      const sim = cosineSim(user, job.features)
      const bonus = riasecBonus(job.riasec, userTop2) * 0.08
      return { ...job, matchPct: Math.round(sim * 100), _rank: sim + bonus }
    })
...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/model/recommend', {
      ...
      body: JSON.stringify({ user_scores: features, top_n: 15 }),
    })
      .then(r => r.json())
      .then(data => {
        if (data.error) throw new Error(data.error)
        // RIASEC 유형 일치도로 재정렬 (모델 코사인 유사도가 99%대로 밀집돼 있어 단순 순위 신뢰 불가)
        const tagged = data.map(item => {
          const riasec = getJobRiasec(item.job_name)
          const bonus = riasecBonus(riasec, userTop2) * 5
          return { ...item, riasec, _rank: item.similarity_score + bonus }
        })
        tagged.sort((a, b) => b._rank - a._rank)
        setCareers(tagged.slice(0, 4).map(item => ({
          title: item.job_name,
          matchPct: Math.round(item.similarity_score),
          schools: item.schools,
          emoji: getJobEmoji(item.job_name),
          riasec: item.riasec,
          desc: '',
        })))
      })
      .catch(() => {
        setCareers(getCareerMatches(scores))
      })
      ...
      <div ref={resultRef} style={{ background: '#F5F3FF', paddingBottom: 8 }}>
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          <div style={{
            ...
            💡 한국직업정보 재직자 데이터 기반 570개 직업 코사인 유사도 분석 결과입니다.<br />
            추천 서버가 꺼진 경우 로컬 데이터로 자동 전환됩니다.
          </div>
        </div>
